...
const ContainerModel = memo(({ path, ...props }) => {
...
const Body = memo(({ id, height, color, roughness, addons, setVehicle, container_color }) => {
    ...
    // Set body color.
    useEffect(() => {
        setObjectMaterials(vehicle.current, color, roughness, undefined, undefined, container_color)
    }, [setObjectMaterials, color, roughness, addons, container_color])
...
const Vehicle = ({ currentVehicle, setVehicle }) => {
    ...
    const container_id = addons.box || null;

    return (
        <group name='Vehicle'>
            <Body key={id} id={id} height={vehicleHeight} color={color} roughness={roughness} addons={addons} setVehicle={setVehicle} container_color={container_color} />
            <Wheels
                rim={rim}
