...
const setMaterials = (material, color, roughness, rim_color, rim_color_secondary, container_color) => {
    ...
    switch (material.name) {
        ...
        case 'rim_secondary':
            ...
            break
        case 'container':
            material.color.setStyle(container_color)
            material.metalness = 0.4
            // material.roughness = roughness
            break
        default:
...
export default function useMaterialProperties() {
    ...
    const setObjectMaterials = useCallback((object, color, roughness, rim_color, rim_color_secondary, container_color) => {
        if (!object) return
        ...
        object.traverseVisible((child) => {
            if (child instanceof Mesh) {
                ...
                materials.forEach((material) => {
                    setMaterials(material, color, roughness, rim_color, rim_color_secondary, container_color)
                })
            }
        })
    }, [])
